...
This notebook attempts to use the Polish data to perform basic data cleaning and define the views and data validation that will be required into the data to provide meaningful visualisations efficiently. It focuses on the occurence data.
...
Now we need to allocate the correct data types. R correctly infers these in general, but let us be explicit because we need to know this for writing our view.
...
One degree of latitude defines a length of about 100km. One degree of longitude defines a varying length depending on the latitude, but it is about 1000km. 
...
If 1 latitude degree gives us 100km, then 1/10th of one degree (one decimal degree) gives us 10km. So we want 5/10ths of a decimal degree. We want to round our latitude to the nearest half degree like this

```{r}
test_vec <- c(52.21380, 53.36764, 53.38855)
round(test_vec*2/1,0)*1/2
```
And for longitude, if one degree of longitude gives us 1000km, then 1/10th degree of longitude gives us 100km, so 1/100th degree of longitude gives us 10km, so we want 5/100th degrees of longitude, roughly.

```{r}
round(test_vec*100/5,0)*5/100
```
Or, as a function
```{r}
...
df_binned <- df_transformed |> 
  mutate(
    ...
    longitude = round_to_the_n_over_d(longitude, 5, 100)
  )
```
...
```
The example shows that the binning is not too bad. Interestingly in this example it moved the points over a border. In some sense this is not desirable, but in others it is. The actual ecosystems cross the borders, it isn't totally incorrect to cross the border. A research team trying to understand biological diversity in a border region would probably get permits to study on both sides of the border. For the global data, we need not even invoke national boundaries at all, so I will classify this rounding as a success.
...
processed_db_data <- raw_db_data |> 
  ...
  mutate(
    ...
    longitude = round(longitude*100/5,0)*5/100
  ) |> 
...
```
Which is very ugly and probably non-optimal SQL. But it is good because it is the result of a workflow we trust.
...
Extra notes
* I realised later that we shouldn't uses the common names in this table because they are irregular, and prevent efficient aggregation. This has been removed in the final SQL code.